...
function diagnosticText(ts: typeof import("typescript"), diagnostic: import("typescript").Diagnostic) {
...
async function run() {
  ...
  try {
    ...
    if (errors.length) {
      throw new Error(errors.map((error) => diagnosticText(ts, error)).join("\n"));
    }
    ...
    if (typeof module.exports.mount !== "function") {
      throw new Error(
        'The demo must export a function named "mount".',
      );
    }
